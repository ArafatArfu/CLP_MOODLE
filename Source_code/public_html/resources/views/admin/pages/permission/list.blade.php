...
<div class="card">
	...
	<div class="card-header border-0 pt-6">
		...
		<div class="card-toolbar">
            ...
            @include('admin.modals.add-permission')
            {{-- @include('admin.modals.view-permission') --}}
            @include('admin.modals.edit-permission')
			<!--end::Modal - Add task-->
		</div>
		<!--end::Card toolbar-->
	</div>
                        ...
                        @foreach ($roles as $role)
                            <td class="text-center">
                                @if($permission->roles->contains('id', $role->id))
                                    ...
                                    <form method="GET" action="{{ url('/admin/revoke-permission/' . $permission->id . '/roles/' . $role->id) }}">
                                        @csrf
                                ...
                                @else
                                    ...
                                    <form method="GET" action="{{ url('/admin/add-permission/' . $permission->id . '/roles/' . $role->id) }}">
                                        @csrf
