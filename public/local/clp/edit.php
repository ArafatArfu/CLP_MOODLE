<?php
// Create / edit a single CLC record from the dashboard.

require_once(__DIR__ . '/../../config.php');
require_once(__DIR__ . '/locallib.php');

use local_clp\form\program_record_form;
...
if ($data = $form->get_data()) {
    ...
    if (!empty($data->id)) {
        $save->id = $data->id;
        $DB->update_record('clp_clc_participants', $save);
        $message = get_string('recordupdated', 'local_clp');
    } else {
        $DB->insert_record('clp_clc_participants', $save);
        $message = get_string('recordcreated', 'local_clp');
    }
...
$title = $id ? get_string('editrecord', 'local_clp') : get_string('addrecord', 'local_clp');
$subtitle = $id
    ? get_string('editrecordsub', 'local_clp', 'CLC')
    : get_string('addrecordsub', 'local_clp', 'CLC');

$schoolsjson = json_encode(array_values(local_clp_distinct_schools()));
