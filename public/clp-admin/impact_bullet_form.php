<?php
// CLP Admin Panel - Impact Bullet Form (Add/Edit)

require_once __DIR__ . '/includes/auth.php';

$page_title = 'Impact Bullet Form';

$db = clp_db_connect();
$bullet = [
    'id' => '',
    'section_key' => 'growth_expansion',
    'bullet_text' => '',
    'display_order' => 0,
    'status' => 'draft'
];
...
if (isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    $stmt = $db->prepare("SELECT * FROM clp_impact_bullets WHERE id = ? LIMIT 1");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $bullet['section_key'] = clp_sanitize($_POST['section_key'] ?? 'growth_expansion');
    ...
    if (empty($bullet['bullet_text'])) {
        ...
        if (!empty($bullet['id'])) {
            $stmt = $db->prepare("UPDATE clp_impact_bullets SET section_key=?, bullet_text=?, display_order=?, status=? WHERE id=?");
            $stmt->bind_param("ssisi", $bullet['section_key'], $bullet['bullet_text'], $bullet['display_order'], $bullet['status'], $bullet['id']);
        } else {
            $stmt = $db->prepare("INSERT INTO clp_impact_bullets (section_key, bullet_text, display_order, status) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssis", $bullet['section_key'], $bullet['bullet_text'], $bullet['display_order'], $bullet['status']);
        }

        if ($stmt->execute()) {
            clp_set_success(!empty($bullet['id']) ? 'Impact bullet updated successfully.' : 'Impact bullet created successfully.');
            $stmt->close();
            $db->close();
            clp_redirect(CLP_ADMIN_URL . '/impact_bullets.php');
        } else {
            clp_set_error('Failed to save impact bullet.');
        }
        $stmt->close();
    }
}
...
<div class="content-area">
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title"><?php echo !empty($bullet['id']) ? 'Edit Impact Bullet' : 'Add New Impact Bullet'; ?></h3>
            <a href="<?php echo CLP_ADMIN_URL; ?>/impact_bullets.php" class="btn btn-primary"><i class="fas fa-arrow-left"></i> Back to List</a>
        </div>
        <form method="POST" action="">
            ...
            <a href="<?php echo CLP_ADMIN_URL; ?>/impact_bullets.php" class="btn btn-primary">Cancel</a>
        </form>
    </div>
</div>
