...
/**
 * Delete a file from Moodle File API by filename and center ID.
 */
function clp_delete_file(string $filearea, string $filename, int $centerId = 0): void {
    $context = \context_system::instance();
    $fs = get_file_storage();

    $files = $fs->get_area_files($context->id, 'local_centermanagement', $filearea, $centerId, 'id', false);
    ...
    if ($centerId > 0) {
        clp_db()->delete_records($table, ['center_id' => $centerId, 'filename' => $filename]);
    } else {
        clp_db()->delete_records_select($table, 'filename = ?', [$filename]);
    }
}

/**
 * Get all filenames for a center and filearea.
 */
function clp_get_uploaded_files(int $centerId, string $filearea): array {
    ...
    $records = clp_db()->get_records($table, ['center_id' => $centerId], 'sortorder ASC, id ASC');
...
/**
 * Delete all files for a center/filearea from both Moodle storage and DB.
 */
function clp_delete_all_files(int $centerId, string $filearea): void {
    ...
    $prefix = clp_db()->get_prefix();
    $table = $prefix . 'local_centermanagement_' . $filearea;
    clp_db()->delete_records($table, ['center_id' => $centerId]);
}
