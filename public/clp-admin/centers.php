...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] === 'delete' && $id > 0) {
        $DB->delete_records('local_centermanagement_sponsors', ['center_id' => $id]);
        $DB->delete_records('local_centermanagement_banner_images', ['center_id' => $id]);
        $DB->delete_records('local_centermanagement_plaque_gallery', ['center_id' => $id]);
        $DB->delete_records('local_centermanagement_school_photo_gallery', ['center_id' => $id]);
        
        // Also delete files from Moodle file pool
        $context = \context_system::instance();
        $fs = get_file_storage();
        foreach (['banner_images', 'plaque_images', 'school_photos'] as $fa) {
            $files = $fs->get_area_files($context->id, 'local_centermanagement', $fa, $id, 'id', false);
            foreach ($files as $file) {
                $file->delete();
            }
        }
        
        $DB->delete_records('local_centermanagement_centers', ['id' => $id]);
